...
class PostgresqlSpec extends AnyFlatSpec with TestContainerForAll {
  ...
  "PostgreSQL data source" should "read table" in withContainers { postgresServer =>
    ...
    spark
      ...
      .option("partitionSize", 9)
      .option("partitioningColumn", "user_id")
      .load()
      .show(50)
  ...
  "PostgreSQL data source" should "write table" in withContainers { postgresServer =>
    ...
    val df = (60 to 70).map(i=>(i.toLong, s"user_$i")).toDF("user_id", "user_name")
  ...
  object Queries {
    lazy val createTableQuery = s"CREATE TABLE $testTableName (user_id BIGINT PRIMARY KEY, user_name VARCHAR(64) NOT NULL);"

    lazy val testValues: String = (1 to 50).map(i => s"($i, 'user_$i')").mkString(", ")
